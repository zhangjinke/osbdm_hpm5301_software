...
Div8r:
  ldhx syncTime pshh txa pulx

         lsrx;
  divide by 8 rora lsrx rora lsrx rora adc #0 bcc bgndTC01;
  round up                                        aix #1;
  save carry to high byte                         bgndTC01 : pshx;
  save                                            bitTime, 16 target BDC clock periods psha

                  lsrx;
  compute 8 clock time rora

    adc #0 bcc bgndTC02;
  round up     aix #1 bgndTC02 :

    pshx;
  save 8 cycle time psha lsrx rora

    adc #0 bcc bgndTC03;
  round up     aix #1 bgndTC03 :

    pshx;
  save 4 cycle time psha

    lsrx;
  compute 2 cycle time rora

    adc #0 bcc           bgndTC04;
  round up               aix #1;
  add carry to high byte bgndTC04 :
  ...
  save 4 cycle time
                              ...
                              pulx;
  pull 8 cycle time pulh sthx bit0Time;
  save 8 cycle time
...
SR_BDM2:
;
  BRSET 3, KBISC, SR_BDM2a;
  finish if reset high
  ...
  already clear;
  clrh;
  1, Set zero pulse length to hold;
  clrx; 1, the output high for a bit time
SR_BDM3:
	brclr	7, TPM2SC, SR_BDM3	;
  5, Await the current bit period end mov #TPMINIT, TPM2SC;
  4, Reset the flag bit sthx          TPM2C0VH;
