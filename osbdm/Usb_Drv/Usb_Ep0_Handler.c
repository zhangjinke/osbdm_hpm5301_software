...
void USB_Transaction_Handler (void)
{
  ...
  //   //  non-EP0 endpoint
  //   if (stat & 0x08)
  //   {           // if this is an IN transaction
  //     asm(nop); // do nothing
  //   }
  //   else
  //   { // else call EPx_OUT handler
...
void USB_StdGetDsc_Handler (void)
{
  if (Setup_Pkt.StdCtl.bmRequestType == 0x80)
  {
    ...
    {
    case DSC_DEV:
      if ((_Serial3 >= 0x30) && (_Serial3 <= 0x39) && (_Serial2 >= 0x30) && (_Serial2 <= 0x39) && (_Serial1 >= 0x30) &&
          (_Serial1 <= 0x39) && (_Serial0 >= 0x30) && (_Serial0 <= 0x39))
      {
...
void USB_StdFeatureReq_Handler (void)
{
  if ((Setup_Pkt.CtlReqT.bFeature == DEVICE_REMOTE_WAKEUP) && (Setup_Pkt.CtlReqT.Recipient == RCPT_DEV))
  {
  ...
  if ((Setup_Pkt.CtlReqT.bFeature == ENDPOINT_HALT) && (Setup_Pkt.CtlReqT.Recipient == RCPT_EP) &&
