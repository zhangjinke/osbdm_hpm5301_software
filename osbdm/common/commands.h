...
#define CMD_READ_MEM \
  0x30 // parameters: 8-bit Memory Type, 32-bit address, 8-bit Size (8/16/32), 32-bit length (number to read)
#define CMD_WRITE_MEM \
  0x31 // parameters: 8-bit Memory Type, 32-bit address, 8-bit Size (8/16/32), 32-bit length (number to write), data ...
...
#define CMD_FILL \
  0x34 // parameters: 8-bit Memory Type, 32-bit address, 8-bit Size (8/16/32), 32-bit length (number to write), data ...
...
#define CMD_CONFIG \
  0x70 // OSBDM-JM60 and target specific configuration.
       // parameters: config parameter and its value


// JTAG commands
#define CMD_JTAG_GOTORESET \
  0x80 // no parameters, takes the TAP to TEST-LOGIC-RESET state, re-select the JTAG target to take TAP back to
       // RUN-TEST/IDLE
#define CMD_JTAG_GOTOSHIFT \
  0x81 // parameters 8-bit path option; path option ==0 : go to SHIFT-DR, !=0 : go to SHIFT-IR (requires the tap to be
       // in RUN-TEST/IDLE)
#define CMD_JTAG_WRITE \
  0x82 // parameters 8-bit exit option, 8-bit count of bits to shift in, and the data to be shifted in (shifted in LSB
       // (last byte) first, unused bits (if any) are in the MSB (first) byte; exit option ==0 : stay in SHIFT-xx, !=0 :
       // go to RUN-TEST/IDLE when finished
#define CMD_JTAG_READ \
  0x83 // parameters 8-bit exit option, 8-bit count of bits to shift out; exit option ==0 : stay in SHIFT-xx, !=0 : go
       // to RUN-TEST/IDLE when finished, returns the data read out of the device (first bit in LSB of the last byte in
       // the buffer)
